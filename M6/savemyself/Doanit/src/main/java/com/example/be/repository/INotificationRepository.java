...
public interface INotificationRepository extends JpaRepository<Notification, Integer> {
    ...
    @Query(value = "SELECT notification_id, content, status, time_notification, title, url, account_id, account_send_notification_id FROM doanit.notification WHERE notification.account_id = ?1  AND notification.status = 0  ORDER BY notification.time_notification DESC", nativeQuery = true)
    List<Notification> findByAccountIdAndStatusIsTrue(Integer accountId);
